...
function saveUserProgress(){
  ...
  (depts||[]).forEach(d=>(d.partitions||[]).forEach(p=>{
    const walkCats = cats => (cats||[]).forEach(c=>{
      ...
      (c.items||[]).forEach(it=>{ if(it.type==='item'&&it.completed) map[it.id]=true; });
    ...
    (d.partitions||[]).forEach(p=>{
      ...
      const walk = cats => (cats||[]).forEach(c=>{
        ...
        (c.items||[]).forEach(it=>{
          ...
          const isDone=!!map[it.id]; t++; if(isDone) dn++;
          items.push({ id:it.id, name:it.title||'خطوة', done:isDone });
        });
...
function applyUserProgress(){
  ...
  (depts||[]).forEach(d=>(d.partitions||[]).forEach(p=>{
    const walkCats = cats => (cats||[]).forEach(c=>{
      ...
      (c.items||[]).forEach(it=>{ if(it.type==='item') it.completed=!!map[it.id]; });
      if(c.subCategories) walkCats(c.subCategories);
    });
    walkCats(p.categories||[]);
  }));
...
function stats(node){
  ...
  const walkCats = cats => cats.forEach(cat=>{
    ...
    (cat.items||[]).forEach(it=>{ if(it.type==='item'){ t++; if(it.completed) d++; } });
...
function locate(id){
  ...
  for(const dep of depts) for(const part of (dep.partitions||[])) for(const cat of (part.categories||[])){
    ...
    const ii=(cat.items||[]).findIndex(it=>it.id===id);
    if(ii>-1) return {node:cat.items[ii], arr:cat.items, index:ii, kind:'item', cat, part, dept:dep};
  }
  return null;
}
...
function toggleCourse(id){
  const r=locate(id); if(!r||(r.kind!=='course'&&r.kind!=='item')) return;
...
function renderSpecialCategory(cat, pc){
  const items=cat.items||[];
  if(!items.length) return '<div class="nt-empty"><div class="nt-empty-ic"><i data-lucide="list-checks"></i></div><p class="nt-empty-txt">لا توجد عناصر بعد</p></div>';
  if(cat.kind==='roadmap'){
    const actionable=items.filter(it=>it.type==='item');
    const done=actionable.filter(it=>it.completed).length;
    const pct=actionable.length?Math.round(done/actionable.length*100):0;
    const groups=[]; let cur={title:'المسار الرئيسي',items:[],info:[]}; groups.push(cur);
    items.forEach(it=>{
      if(it.type==='heading'){ cur={title:it.title,items:[],info:[]}; groups.push(cur); }
      else if(it.type==='info') cur.info.push(it.title);
      else cur.items.push(it);
    });
    const useful=groups.filter(g=>g.items.length||g.info.length);
    let n=0;
    return '<div class="rm-metro">'+
      '<div class="metro-summary"><div><span class="metro-kicker">ENGINEERING ROUTE</span><b>خريطة المسار</b></div><div class="metro-progress"><span>'+done+' / '+actionable.length+'</span><i><em style="width:'+pct+'%"></em></i><strong>'+pct+'%</strong></div></div>'+
      '<div class="metro-legend"><span><i class="lg-done"></i>مكتمل</span><span><i class="lg-current"></i>المسار</span><span><i class="lg-next"></i>قادم</span></div>'+
      useful.map((g,gi)=>'<section class="metro-line '+(gi?'branch':'main')+'">'+
        '<div class="metro-phase"><span>'+(gi?'فرع '+String(gi).padStart(2,'0'):'المسار الرئيسي')+'</span><h4>'+esc(g.title)+'</h4></div>'+
        '<div class="metro-track">'+g.items.map(it=>{ n++; return '<div class="metro-station '+(it.completed?'done':'')+'"><button class="metro-stop" style="--acc:'+pc+'" onclick="toggleCourse(\''+it.id+'\')"><i data-lucide="check"></i><span>'+String(n).padStart(2,'0')+'</span></button><div class="metro-card"><b>'+esc(it.title)+'</b></div></div>'; }).join('')+'</div>'+
        (g.info.length?'<div class="metro-notes">'+g.info.map(x=>'<p><i data-lucide="info"></i>'+esc(x)+'</p>').join('')+'</div>':'')+
      '</section>').join('')+
    '</div>';
  }
  return '<div class="cl-list">'+items.map(it=>{
    if(it.type==='heading') return '<h4 class="cl-heading">'+esc(it.title)+'</h4>';
    if(it.type==='info') return '<p class="cl-info"><i data-lucide="info"></i>'+esc(it.title)+'</p>';
    return '<div class="cl-item '+(it.completed?'done':'')+'"><button class="chk '+(it.completed?'done':'')+'" style="--acc:'+pc+'" onclick="toggleCourse(\''+it.id+'\')"><i data-lucide="check"></i></button><span>'+esc(it.title)+'</span></div>';
  }).join('')+'</div>';
}
...
function renderCategory(cat, part, ci, total, pc){
  ...
  const cnt=cat.kind ? (cat.items||[]).filter(it=>it.type==='item').length : (cat.courses||[]).length + (cat.subCategories||[]).reduce((a,s)=>a+(s.courses||[]).length,0);
...
function renderCategoryDetail(){
  ...
  const ccnt=cat.kind ? (cat.items||[]).filter(it=>it.type==='item').length : (cat.courses||[]).length + (cat.subCategories||[]).reduce((a,s)=>a+(s.courses||[]).length,0);
  ...
  wrap.innerHTML=`
    ...
    <div class="sheet" style="--acc:${pc}">
      ...
      <div class="nt-tabs">
        <button type="button" onclick="setCatTab('content')" class="nt-tab ${currentCatTab==='content'?'on-content':''}"><i data-lucide="${cat.kind==='roadmap'?'route':cat.kind==='checklist'?'list-checks':'layers'}" class="w-4 h-4"></i> ${cat.kind==='roadmap'?'الخريطة':cat.kind==='checklist'?'القائمة':'المحتوى'} (${ccnt})</button>
  ...
  if(cat.kind) body.innerHTML=renderSpecialCategory(cat,pc);
  ...
  (cat.subCategories||[]).forEach(sub=>{
    ...
    if(!cat.kind && !(cat.courses||[]).length && !(cat.subCategories||[]).length){
