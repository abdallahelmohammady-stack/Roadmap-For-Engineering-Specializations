...
function renderSpecialCategory(cat, pc){
  ...
  if(cat.kind==='roadmap'){
    let n=0;
    return '<div class="rm-map">'+items.map(it=>{
      if(it.type==='heading') return '<h4 class="rm-heading">'+esc(it.title)+'</h4>';
      if(it.type==='info') return '<p class="rm-info">'+esc(it.title)+'</p>';
      n++;
      return '<div class="rm-node '+(it.completed?'done':'')+'"><button class="chk '+(it.completed?'done':'')+'" style="--acc:'+pc+'" onclick="toggleCourse(\''+it.id+'\')"><i data-lucide="check"></i></button><span class="rm-num">'+String(n).padStart(2,'0')+'</span><b>'+esc(it.title)+'</b></div>';
    }).join('')+'</div>';
  }
